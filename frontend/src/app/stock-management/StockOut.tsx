...
import { ArrowRight, Check, Package, ShoppingCart, Eye, X, ChevronDown, ChevronUp, Plus, Printer, Download } from 'lucide-react';
...
import { CreateStockOutDto, StockOutState, StockOutItemState } from '@/types/inventory/stock-out';
import { StockService } from '@/services/inventory/stock.service';
import { PaymentService } from '@/services/finance/payment.service';
import api from '@/lib/axios';
import { useAuthStore } from '@/store/useAuthStore';

export class StockOut extends Component<{}, StockOutState> {
  constructor(props: {}) {
    super(props);
    this.state = {
      currentStep: 1,
      ...
      notes: '',
      items: [{ productId: '', quantity: '', unitPrice: 0, availableStock: 0 }],
      warehouseStock: [],
      isSubmitting: false,
      error: null,
      selectedViewId: null
  ...
  async componentDidMount() {
    try {
      ...
      this.setState({ 
        ...
        if (this.state.warehouseId) this.recalculateAllStocks();
      });
    } catch (error) {
  ...
  handleApprove = async (id: string) => {
    try {
      await StockOutService.approveStockOut(id);
      toast.success('Duyệt phiếu xuất kho thành công');
      this.loadStockOuts();
    } catch (e: any) {
  ...
  handleCancelStockOut = async (id: string) => {
    if (!confirm('Bạn chắc chắn muốn huỷ phiếu này?')) return;
    try {
      await StockOutService.cancelStockOut(id);
  ...
  handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    this.setState({ [name || id]: value } as any, () => {
      if (name === 'warehouseId' || id === 'warehouse') {
        this.recalculateAllStocks();
      }
    });
  };
  ...
  recalculateAllStocks = async () => {
    ...
    try {
      const stocks = await StockService.getStock({ warehouseId: this.state.warehouseId });
      // Chỉ lấy những sản phẩm có số lượng > 0
      const availableStocks = stocks.filter((s: any) => s.quantity > 0);
      
      const newItems = this.state.items.map(item => {
        if (!item.productId) return item;
        const stockItem = stocks.find((s: any) => s.productId === item.productId);
        return { ...item, availableStock: stockItem ? stockItem.quantity : 0 };
      });
      this.setState({ items: newItems, warehouseStock: availableStocks });
    } catch (err) {
      console.error('Error recalculating stocks', err);
    }
  };

  handleItemChange = async (index: number, field: string, value: string) => {
    const newItems = [...this.state.items];
    const numValue = value === '' ? '' : Number(value);
    
    newItems[index] = { ...newItems[index], [field]: field === 'productId' ? value : numValue };
    
    // Automatically load stock availability and price if product selected
    if (field === 'productId' && value && this.state.warehouseId) {
       try {
         const warehouseId = this.state.warehouseId;
         const stocks = await StockService.getStock({ warehouseId });
         const stockItem = stocks.find((s: any) => s.productId === value);
         newItems[index].availableStock = stockItem ? stockItem.quantity : 0;
         // Auto-fill giá bán từ product.price
         const product = this.state.products.find((p: any) => p.id === value);
         if (product?.price) newItems[index].unitPrice = Number(product.price);
       } catch (err) {
  ...
  addItem = () => {
    this.setState({ items: [...this.state.items, { productId: '', quantity: '', unitPrice: 0, availableStock: 0 }] });
  };

  nextStep = async () => {
    const { currentStep, items } = this.state;
    this.setState({ error: null });

    if (currentStep === 1) {
      if (!this.state.customerId) {
        this.setState({ error: 'Please select a customer.' });
        return;
      }
      if (!this.state.warehouseId) {
        this.setState({ error: 'Please select a warehouse.' });
        return;
      }
      ...
      const invalidStockItem = validItems.find(i => Number(i.quantity) > (i.availableStock || 0));
      if (invalidStockItem) {
        const p = this.state.products.find(p => p.id === invalidStockItem.productId);
        this.setState({ error: `Quantity for "${p?.name}" exceeds available stock! (${invalidStockItem.availableStock} available)` });
        return;
      }

      this.setState({ items: validItems, currentStep: currentStep + 1 });
    } else if (currentStep === 3) {
      await this.submitStockOut();
    }
  };

  submitStockOut = async () => {
    this.setState({ isSubmitting: true, error: null });
    const { referenceCode, items, warehouseId, customerId, notes } = this.state;
    
    try {
      const dto: CreateStockOutDto = {
        customerId,
        warehouseId,
        referenceCode: referenceCode || `SO-${Date.now()}`,
        items: items.map(i => ({
          productId: i.productId,
          quantity: Number(i.quantity),
          price: Number((i as any).unitPrice || 0)
        })),
        notes
      };

      // Chỉ tạo phiếu (PENDING) — việc duyệt/hoàn thành do người có quyền thực hiện trong danh sách bên dưới
      await StockOutService.createStockOut(dto);
          ...
          <motion.div
            ...
            <div className="mb-4">
              ...
              <select
                ...
                {(this.state as any).customers?.map((c: any) => (
        ...
        return (
          <motion.div
              ...
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-700/50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Available</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Qty Out</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Unit Price (Giá bán)</th>
                  ...
                  <tr key={index}>
                    ...
                    <td className="px-4 py-3">
                      <input type="number" min="1" max={item.availableStock} value={item.quantity}
                        ...
                        className={`w-full text-sm border-0 focus:ring-0 bg-transparent dark:text-white ${Number(item.quantity) > (item.availableStock || 0) ? 'text-red-500 font-semibold' : ''}`}
                        placeholder="0" />
                    </td>
                    <td className="px-4 py-3">
                      <input type="number" min="0" step="1000" value={item.unitPrice}
                        onChange={(e) => this.handleItemChange(index, 'unitPrice', e.target.value)}
                        className="w-full text-sm border-0 focus:ring-0 bg-transparent dark:text-white"
                        placeholder="0" />
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                      {((Number(item.quantity) || 0) * (item.unitPrice || 0)).toLocaleString()}đ
                    </td>
                  </tr>
                  ))}
                </tbody>
            ...
            <div className="flex justify-end mb-2">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300 mr-2">Tổng:</span>
              <span className="text-sm font-semibold text-gray-800 dark:text-white">
                {this.state.items.reduce((acc, item) => acc + ((Number(item.quantity) || 0) * (item.unitPrice || 0)), 0).toLocaleString()}đ
              </span>
            </div>
            <motion.button
              whileHover={{
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <p className="text-sm text-yellow-700 dark:text-yellow-400">
                    This action will reduce your inventory. Please review all
    ...
    return (
      <div className="mt-8">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Danh sách phiếu xuất</h3>
          <button 
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700/50">
              <tr>
                {['Mã phiếu','Khách hàng','Kho','Ngày tạo','Trạng thái','Thanh toán','Hành động'].map(h => (
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {paged.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-6 text-center text-gray-500 text-sm">Không có phiếu xuất nào</td></tr>
              )}
              {paged.map((so: any) => {
                const warehouseName = so.warehouseName || warehouses.find((w: any) => w.id === so.warehouseId)?.name || so.warehouseId;
                const customerName = so.customerName || customers.find((c: any) => c.id === so.customerId)?.name || so.customerId;
                ...
                return (
                  <tr key={so.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{customerName}</td>
                    ...
                    <td className="px-4 py-3 flex gap-2 flex-wrap">
                      ...
                      <button onClick={() => this.handlePrint(so.id)}
                      ...
                      {((so.status || '').toUpperCase() === 'PENDING' || (so.status || '').toUpperCase() === 'APPROVED') && (
                        <button onClick={() => this.handleCancelStockOut(so.id)}
                  ...
                  return (
                    <div>
                      <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
                        ...
                        <div><span className="text-gray-500">Người nhận:</span> <span className="font-medium text-gray-900 dark:text-white">{customers.find((c: any) => c.id === so.customerId)?.name || so.customerId}</span></div>
                      </div>
                      ...
                      <div className="mt-8">
                        <NoteSection entityType="stock_out" entityId={so.id} />
                      </div>
                    </div>
                  );
                })()}
        ...
        {(this.state as any).showPaymentModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 overflow-y-auto">
            ...
            >
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Xác nhận thu tiền</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Vui lòng chọn phương thức ghi nhận tiền cho phiếu xuất này.</p>
              ...
              <div className="flex gap-3 justify-end mt-8">
                ...
                <button 
                  ...
                  disabled={(this.state as any).isProcessingPayment}
                >
                  {(this.state as any).isProcessingPayment ? 'Đang xử lý...' : 'Xác nhận thu tiền'}
                </button>
              </div>
            </motion.div>
      ...
      <div>
        ...
        <motion.div
          ...
          transition={{
            ...
            <h2 className="text-xl font-semibold text-gray-800 dark:text-white mb-2">
              Phếu xuất đã được tạo!
            </h2>
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Phếu xuất đang ở trạng thái <strong>Chờ duyệt (PENDING)</strong>. Người có quyền có thể duyệt và hoàn thành phiếu trong danh sách bên dưới.
            </p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden mb-8">
            ...
            >
              <div className="flex items-center gap-3">
                 ...
                 <h2 className="text-lg font-semibold text-gray-800 dark:text-white">Tạo phiếu xuất kho</h2>
              </div>
            ...
            <div className="p-6">
              ...
              <div className="flex justify-between mt-8">
                ...
                (this.state.isSubmitting ? 'Processing...' : 'Confirm Stock Out')
                }
                </motion.button>
              </div>
            </div>
        ...
        {(this.state as any).printingId && (
            <div className="hidden print:block fixed inset-0 z-[9999] bg-white">
                {(() => {
                    const so = this.state.stockOuts.find((s: any) => s.id === (this.state as any).printingId);
                    if (!so) return null;
                    const customer = this.state.customers.find(c => c.id === so.customerId);
                    return <SlipPrintLayout type="OUT" data={{ ...so, customerName: customer?.name }} />;
                })()}
            </div>
        )}
      </div>
