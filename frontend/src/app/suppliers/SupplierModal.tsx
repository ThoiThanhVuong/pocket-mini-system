...
export class SupplierModal extends Component<SupplierModalProps, SupplierModalState> {
  ...
  validate = (data: Record<string, string>) => {
    ...
    if (!data.name?.trim()) {
      ...
      errors.name = 'Tên nhà cung cấp chỉ được chứa chữ cái và khoảng trắng';
    } else if (data.name.trim().length < 2) {
      errors.name = 'Tên phải có ít nhất 2 ký tự';
    }

    if (!data.contactPerson?.trim()) {
      errors.contactPerson = 'Người liên hệ không được để trống';
    } else if (!NAME_REGEX.test(data.contactPerson.trim())) {
      errors.contactPerson = 'Tên người liên hệ chỉ được chứa chữ cái và khoảng trắng';
    }
